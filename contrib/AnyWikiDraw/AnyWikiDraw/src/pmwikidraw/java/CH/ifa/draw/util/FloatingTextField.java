...
public  class FloatingTextField extends Object {
    ...
    private TextComponent fEditWidget;
    private boolean singleRowMode = false;

    private Container   fContainer;
    public FloatingTextField(int rows, int cols) {
      
      if(rows>1) {
        fEditWidget = new TextArea(rows, cols);
        singleRowMode = false;
      }
      else {
        singleRowMode = true;
        fEditWidget = new TextField(cols);
      }
    }
    ...
    public Dimension getPreferredSize(int rows, int cols) {
      if(singleRowMode) {
        ...
        return ( (TextField) fEditWidget).getPreferredSize(cols);
      } 
      else {
        return ( (TextArea) fEditWidget).getPreferredSize(rows, cols);
      }
