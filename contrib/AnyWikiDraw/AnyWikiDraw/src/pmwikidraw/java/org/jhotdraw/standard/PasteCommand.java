...
public class PasteCommand extends FigureTransferCommand {
	...
	public void execute() {
		...
		if (selection != null) {
			...
			Point targetPoint = new Point(lastClick.x-r.x, lastClick.y-r.y);
			
			PointConstrainer constrainer =  view().getConstrainer();
			if(constrainer!=null) {
			  constrainer.constrainPoint(targetPoint);
			}
			
			// get an enumeration of inserted figures
			FigureEnumeration fe = insertFigures(getUndoActivity().getAffectedFigures(), targetPoint.x, targetPoint.y);
