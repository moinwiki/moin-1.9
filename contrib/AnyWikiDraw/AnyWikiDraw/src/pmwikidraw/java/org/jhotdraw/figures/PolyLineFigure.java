...
//	protected void setFrameColor(Color c) {
	...
	public String getMap() {
		String sensitive = (String)getAttribute(FigureAttributeConstant.URL);
		if (sensitive == null || sensitive.length() == 0)
			return "";
		try {
			sensitive = PmWikiDrawURLEncoder.decode(sensitive);
		} catch (Exception e) {}
		
		boolean first  = true;
		String forwards = "", backwards = "";
		for (int i = 1; i < fPoints.size(); i++) {
			Point p1 = (Point)fPoints.get(i-1);
			Point p2 = (Point)fPoints.get(i);
