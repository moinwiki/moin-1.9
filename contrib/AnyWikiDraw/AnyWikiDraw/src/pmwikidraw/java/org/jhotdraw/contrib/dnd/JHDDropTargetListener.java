		...
		if (dtde.isDataFlavorSupported(DNDFiguresTransferable.DNDFiguresFlavor) == true) {
			...
			if ((dtde.getDropAction() & DnDConstants.ACTION_COPY_OR_MOVE) != 0 ) {
				...
				try { /* protection from a malicious dropped object */
					...
					//CJ: Modified to take into account grid constraints when dropping.
					PointConstrainer constrainer = view().getConstrainer();
					if(constrainer!=null) {
						Point constrained = constrainer.constrainPoint(new Point(dx,dy));
						dx = constrained.x;
						dy = constrained.y;
					}
...
////////////////////////////////////////////////////////////////////////////////
	...
	protected boolean supportDropTargetDragEvent(DropTargetDragEvent dtde) {
		...
		if (dtde.isDataFlavorSupported(DNDFiguresTransferable.DNDFiguresFlavor) == true) {
			//CJ: Modification to disallow dnd onto some drawingviews.
